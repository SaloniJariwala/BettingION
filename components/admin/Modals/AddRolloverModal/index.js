...
const AddRolloverModal = ({ show, setShow, userDetails, setFlag }) => {
    ...
    const handleRolloverCredit = async () => {
        setErrorMessage("");
        const authKey = sha1(
            process.env.NEXT_PUBLIC_AUTH_KEY +
                `action=credit&remote_id=${userDetails.remoteId}&token=${
    ...
    return (
        <AdminModal show={show} closeModal={handleClose}>
            <h3 className="h3-title modal_title">Add Rollover</h3>

            <FormProvider {...methods}>
                <form
                    action=""
                    className="add_credit_form"
                    onSubmit={methods.handleSubmit(handleRolloverCredit)}>
...
export default AddRolloverModal;

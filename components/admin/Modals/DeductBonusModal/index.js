...
const DeductBonusModal = ({ show, setShow, userDetails, setFlag }) => {
    ...
    const handleBonusDebit = async () => {
        setErrorMessage("");
        const authKey = sha1(
            process.env.NEXT_PUBLIC_AUTH_KEY +
                `action=debit&remote_id=${process.env.NEXT_PUBLIC_REMOTE_ID}/_${userDetails?.userId}&token=${
    ...
    return (
        <AdminModal show={show} closeModal={handleClose}>
            <h3 className="h3-title modal_title">Deduct Bonus</h3>

            <FormProvider {...methods}>
                <form action="" className="add_credit_form" onSubmit={methods.handleSubmit(handleBonusDebit)}>
...
export default DeductBonusModal;

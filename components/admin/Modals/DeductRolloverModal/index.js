...
const DeductRolloverModal = ({ show, setShow, userDetails, setFlag }) => {
    ...
    const handleRolloverDebit = async () => {
        setErrorMessage("");
        const authKey = sha1(
            process.env.NEXT_PUBLIC_AUTH_KEY +
                `action=debit&remote_id=${userDetails.remoteId}&token=${
                    process.env.NEXT_PUBLIC_TOKEN
                }&currency=${userDefaultCurrency?.currencyAbrv}&casino=${
                    process.env.NEXT_PUBLIC_CASINO
                }&amount=${Number(amount)}&sender_id=${JSON.parse(localStorage.getItem("User"))?.remoteId}`
        );
        await axios
            .get(
                `${process.env.NEXT_PUBLIC_API_DOMAIN}/casinos-admin/api?action=debit&remote_id=${
                    userDetails.remoteId
                }&token=${process.env.NEXT_PUBLIC_TOKEN}&currency=${
                ...
                }&authKey=${authKey}&status=&txnAction=rolloverCreditDebit`
            )
            .then((response) => {
    ...
    return (
        <AdminModal show={show} closeModal={handleClose}>
            <h3 className="h3-title modal_title">Deduct Rollover</h3>

            <FormProvider {...methods}>
                <form
                    action=""
                    className="add_credit_form"
                    onSubmit={methods.handleSubmit(handleRolloverDebit)}>
...
export default DeductRolloverModal;

...
const FinanceAgentOperation = (props) => {
        ...
        return (
            <div className="table_btn_group form_right_group">
                <ul>
                    ...
                    <li>
                        <NextTooltip title={adminLanguageData?.common_finance_module?.tooltip_surcharge?.value}>
                            <button
                                type="button"
                                className="sec_btn icon_btn balance_action"
                                onClick={() => {
    ...
    const getData = () => {
        ...
        axios
            .post(
                `${process.env.NEXT_PUBLIC_API_DOMAIN}/finance-user/actions?action=finance-user-list&type=agent&token=${process.env.NEXT_PUBLIC_TOKEN}&casino=${
                ...
                if (response.data?.status === 200) {
                    ...
                    if (response.data?.data?.users?.length > 0) {
                        setData([
                            ...
                            {
                                id: "footer",
                                username: <b>{adminLanguageData?.common_finance_module?.row_text_total_payment?.value}</b>,
                                role: "",
    ...
    const columns = [
        {
            name: adminLanguageData?.finance_agent_operation_page?.table_cell_user?.value,
            minWidth: "200px",
            grow: 1,
            selector: (row) => row.username || row.id,
        },
        {
            name: adminLanguageData?.finance_agent_operation_page?.table_cell_balance?.value,
            grow: 1,
            selector: (row) =>
        ...
        {
            name: adminLanguageData?.finance_agent_operation_page?.table_cell_actions?.value,
            grow: 1,
            minWidth: "210px",
    ...
    return (
        <>
                ...
                <div className="user_main_sec">
                    <div className="title_bar">
                        <Row className="align-items-center">
                            <Col lg={6}>
                                <div className="title">
                                    <Title>{adminLanguageData?.finance_agent_operation_page?.finance_agent_operation_page_title?.value}</Title>
                                </div>
                            </Col>
                        </Row>
                    </div>
                                                        ...
                                                        <div id="finance_operations_filter" className="dataTables_filter">
                                                            <label>
                                                                <input
                                                                    ...
                                                                    placeholder={adminLanguageData?.finance_agent_operation_page?.username_placeholder?.value}
            ...
            <CollectModal show={collectModal} setShow={setCollectModal} userDetails={userDetails} type="agent" renderData={renderData} />
            <PlayOutModal show={playOutModal} setShow={setPlayOutModal} userDetails={userDetails} type="agent" renderData={renderData} />
            <DiscountModal show={discountModal} setShow={setDiscountModal} userDetails={userDetails} type="agent" renderData={renderData} />
            <SubChargeModal show={subChargeModal} setShow={setSubChargeModal} userDetails={userDetails} type="agent" renderData={renderData} />
        </>
    );
};
...
export async function getServerSideProps() {
    return {
        props: {
            title: "Finance agent operation",
            description: "Finance agent operation",
        },
    };
}

export default FinanceAgentOperation;

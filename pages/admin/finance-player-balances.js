...
const FinancePlayerBalances = (props) => {
    const methods = useForm();
    const { userDefaultCurrency } = BalanceState();
    const { adminLanguageData } = AdminLanguageState();
    const [childPlayers, setChildPlayers] = useState([]);
    ...
    useEffect(() => {
        ...
        axios
            .get(
                ...
                }&hide=false&accountType=player&isFlat=true&authKey=${authkey}`
            )
            .then(async (response) => {
                if (response.data?.status) {
                    setChildPlayers(response.data?.data?.child?.filter((item) => item.accountType === "player"));
                }
            })
            .catch((error) => {
    ...
    const columns = [
        {
            name: adminLanguageData?.finance_player_balances_page?.table_cell_date?.value,
            grow: 1,
            minWidth: "190px",
            selector: (row) => <span className={getClassName(row.operation)}>{getDateAndTime(row.createdAt)}</span>,
        },
        {
            name: adminLanguageData?.finance_player_balances_page?.table_cell_user?.value,
            grow: 1,
            selector: (row) => <span className={getClassName(row.operation)}>{row.username}</span>,
        },
        {
            name: adminLanguageData?.finance_player_balances_page?.table_cell_operation?.value,
            grow: 1,
            minWidth: "100px",
            maxWidth: "400px",
            selector: (row) => (
        ...
        {
            name: adminLanguageData?.finance_player_balances_page?.table_cell_amount?.value,
            grow: 1,
            maxWidth: "200px",
            selector: (row) => (
                <span
    ...
    const getData = async () => {
        ...
        await axios
            .post(
                `${process.env.NEXT_PUBLIC_API_DOMAIN}/finance-user/actions?action=finance-user-txns&type=player-txns&token=${process.env.NEXT_PUBLIC_TOKEN}&casino=${
                    ...
                    if (response.data?.data?.txns?.length > 0) {
                        setData([
                            ...response.data?.data?.txns,
                            {
                                ...
                                operation: adminLanguageData?.finance_player_balances_page?.table_row_total?.value,
                                transactionId: "",
                                createdAt: "",
                                balance: "",
                                image: "",
    ...
    return (
        <>
            ...
            <AdminLayout>
                <div className="user_main_sec">
                    <div className="title_bar">
                        <Row className="align-items-center">
                            <Col lg={6}>
                                <div className="title">
                                    <Title>{adminLanguageData?.finance_player_balances_page?.finance_player_balances_page_title?.value}</Title>
                                </div>
                            </Col>
                        </Row>
                    </div>
                                            ...
                                            <Row className="g-10">
                                                <Col lg={3} sm={6}>
                                                    <div className="form_input_wp form-element">
                                                        ...
                                                        >
                                                            <option defaultValue="All Players" value="all">
                                                                {adminLanguageData?.finance_player_balances_page?.all_player_options?.value}
                                                            </option>
                                                            {childPlayers?.map((item) => (
                                                ...
                                                <Col lg={3} sm={6}>
                                                    <div className="form_input_wp form-element">
                                                        ...
                                                        >
                                                            ...
                                                            <option value="Add Credit">{adminLanguageData?.common_finance_module?.addcredit_option?.value}</option>
                                                            <option value="Remove Credit">{adminLanguageData?.common_finance_module?.remove_credit_option?.value}</option>
                                                        </select>
                                                        <i className="far fa-angle-down"></i>
                                                    </div>
                                                </Col>
                ...
                <AdminModal className="light_box_modal" show={open} closeModal={() => setOpen(false)} size="lg">
                    {!modalImage ? <Loader /> : <Image src={modalImage} alt="modal" height={1500} width={1500} />}
                </AdminModal>
            </AdminLayout>
        </>
    );
...
export async function getServerSideProps() {
    return {
        props: {
            title: "Finance player balances",
            description: "Finance player balances",
        },
    };
}

export default FinancePlayerBalances;

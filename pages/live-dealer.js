...
const LiveDealer = (props) => {
    ...
    const fetchGames = () => {
        ...
        let url = `${
            ...
            pagePerProvider === 1 ? 1 : page
        }&provider=${providerId}&gameType=LiveGames&gameTypeSecondary=${gameType}&gameSearch=`;
    ...
    useEffect(() => {
        if (page !== 1) {
            setOnPageLoad(false);
            let url = `${process.env.NEXT_PUBLIC_API_DOMAIN}/games/admin/getAllCombineAPI?action=available_games&token=${process.env.NEXT_PUBLIC_TOKEN}&casino=${process.env.NEXT_PUBLIC_CASINO}&page=${page}&provider=${providerId}&gameType=LiveGames&gameTypeSecondary=${gameType}&gameSearch=`;
            if (localStorage.getItem("User")) {
                url += `&remoteId=${JSON.parse(localStorage.getItem("User"))?.remoteId}`;
            }
            axios
    ...
    useEffect(() => {
        ...
        axios
            .get(
                `${process.env.NEXT_PUBLIC_API_DOMAIN}/api/slider-slide/get-slides-list?token=${process.env.NEXT_PUBLIC_TOKEN}&casino=${process.env.NEXT_PUBLIC_CASINO}&authKey=${authKey}&slug=live-dealer&status=true&type=offer`
            )
            .then((response) => {
    ...
    useEffect(() => {
        ...
        axios
            .get(
                `${process.env.NEXT_PUBLIC_API_DOMAIN}/api/slider-slide/get-slides-list?token=${process.env.NEXT_PUBLIC_TOKEN}&casino=${process.env.NEXT_PUBLIC_CASINO}&authKey=${authKey}&slug=live-dealer&status=true&type=banner`
            )
            .then((response) => {
                if (response.data?.status === 200) {
